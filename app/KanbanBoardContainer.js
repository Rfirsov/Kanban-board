...
class KanbanBoardContainer extends Component {
  ...
  addTask(cardId, taskName) {
  	...
  	// Create new object and push new task to the array of tasks
  	...
  	// Call the API to add task on the server
  ...
  toggleTask(cardId, taskId, taskIndex) {
	...
	updateCardStatus(cardId, listId) {
		...
		//Only proceed if hoverting over a list
    ...
    if (cardId !== afterId) {
      ...
      //Use splice to remove card and reinsert it the new index
      this.setState(update(this.state, {
      	cards: {
      		$splice: [
      			[cardIndex, 1],
